...
export default class Index extends React.Component {
  ...
  async onRefresh() {
    ...
    const response = await fetch('http://127.0.0.1:8000/fact/quote', {headers})
  ...
  async onSubmitAdd () {
    ...
    const headers = {"Authorization": 'Bearer ' + window.localStorage.getItem("token")}
    const response = await fetch('http://127.0.0.1:8000/fact/quote', {method: 'POST', body, headers})
    const json = await response.json()
  ...
  async onSubmitEdit () {
    ...
    const headers = {"Authorization": 'Bearer ' + window.localStorage.getItem("token")}
    const response = await fetch('http://127.0.0.1:8000/fact/quote/' + edit.id, {method: 'PUT', body, headers})
    const json = await response.json()
  ...
  async onSubmitDelete () {
    ...
    const headers = {"Authorization": 'Bearer ' + window.localStorage.getItem("token")}
    const response = await fetch('http://127.0.0.1:8000/fact/quote/' + data.id, {method: 'DELETE', body, headers})
    const json = await response.json()
